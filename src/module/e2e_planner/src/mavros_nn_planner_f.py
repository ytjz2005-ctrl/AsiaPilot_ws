...
Note: Drone always flies forward
"""
...
class E2EMavrosPlanner:
    def __init__(self):
        rospy.init_node("e2e_mavros_planner_f")
        ...
        self.control_timer = rospy.Timer(rospy.Duration(1.0 / 30.0), self.publish_attitude_timer_cb)
    ...
    def publish_attitude_timer_cb(self, event):
        """Timer callback: continuously send attitude commands (30Hz) to maintain OFFBOARD mode"""
        if not self.current_state.connected:
            return
            
        att = AttitudeTarget()
        att.header.stamp = rospy.Time.now()
        # Ignore angular rates, only control attitude (quaternion) and thrust
        att.type_mask = AttitudeTarget.IGNORE_ROLL_RATE \
                      | AttitudeTarget.IGNORE_PITCH_RATE \
                      | AttitudeTarget.IGNORE_YAW_RATE
        
        # Fill in the latest desired values computed by the neural network
    ...
    def depth_cb(self, data):
        ...
        # Set target velocity vector (always fly in the x direction)
        target_v = torch.tensor([2000., 0.0, 0.0]) 
        target_v_norm = torch.norm(target_v, 2, -1, keepdim=True)
        max_speed = torch.tensor([self.target_speed])
